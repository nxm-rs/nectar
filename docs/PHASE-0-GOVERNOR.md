...
| Module | Item | Verdict |
|---|---|---|
| `window.rs` | `Window` (slot count + Little's-law sizing) | **Keep** — this is the "thin governor". |
...
| `put_sink.rs` | `PutSink` (bounded put window over `FuturesUnordered`) | **Keep** — already `FuturesUnordered` + admission. |
...
Two viable homes once `nectar-kernel` (a name that reads as "executor") retires.

- **(A) Rename `nectar-kernel` -> `nectar-governor`.**
...
- `BoxFuture` still centralizes in `nectar-tasks` under (A) — that is #611 and is independent of the home choice.

Distributing the governor into each consumer (no shared crate) is rejected: it re-scatters the exact per-walker hand-rolling the shared `Admission` predicate removed, and contradicts the roadmap's "one bounded-concurrency substrate" standard.

## 3. Consumer rewire surface

Eleven files across five crates import from `nectar-kernel`.
Only the `Driver`/`WalkPolicy` users carry real work; the rest is a one-line import-path change (`nectar_kernel` -> `nectar_governor`).
...
Sanctioned `Waker::noop` fast paths stay: `file/src/sync.rs`, `file/src/read/cancel.rs`, `put_sink.rs`, and the `postage-issuer` bridge/stamp drivers.
There is no `RawWaker` in the tree today, so no standing lint is warranted.

## 5. Sequencing

1. **#611 first** — centralize `BoxFuture` in `nectar-tasks`, delete `kernel/src/future.rs`, repoint the consumers.
   Cheap (`effort/hours`), and it shrinks the #610 diff.
2. **#610** — rename the crate, delete `driver.rs`, rebuild the eight `Driver`/`WalkPolicy` sites onto `FuturesUnordered` + `Admission`, repoint the import-path-only sites.
