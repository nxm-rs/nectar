...
use nectar_manifest_sim::store02::OldCountingStore;
...
fn build_10(keys: &[GenKey]) -> ChunkAddress {
    ...
    *run(b.build(&store)).unwrap().root()
}

fn build_02(keys: &[GenKey]) -> ChunkAddress02 {
    let store = OldCountingStore::new();
    let mut m: PlainManifest<&OldCountingStore> = Manifest::new(&store);
    for k in keys {
        run(m.add(&k.path, value_addr(k.path.as_bytes()))).unwrap();
    }
    run(m.save()).unwrap()
}
...
fn bench_get(c: &mut Criterion) {
    ...
    for corpus in Corpus::all() {
        for &scale in &OP_SCALES {
            ...
            let root10 = *run(b10.build(&store10)).unwrap().root();
            let reader10: Reader<&CountingStore<StandardChunkSet>, V1> = Reader::new(&store10);
            let key10 = Key::from(probe.raw.as_slice());
            g.bench_with_input(
                BenchmarkId::from_parameter(bench_id(F10, corpus.name(), scale)),
                &scale,
                |b, _| b.iter(|| run(reader10.get(&root10, &key10)).unwrap()),
            );

            // 0.2
            let store02 = OldCountingStore::new();
            let root02 = {
                let mut m: PlainManifest<&OldCountingStore> = Manifest::new(&store02);
                for k in &keys {
                    run(m.add(&k.path, value_addr(k.path.as_bytes()))).unwrap();
                }
                run(m.save()).unwrap()
            };
            let mut reader02: PlainManifest<&OldCountingStore> = Manifest::open(root02, &store02);
            let path = probe.path.clone();
            g.bench_with_input(
                BenchmarkId::from_parameter(bench_id(F02, corpus.name(), scale)),
                &scale,
                |b, _| b.iter(|| run(reader02.lookup(&path)).unwrap()),
            );
        }
    }
    g.finish();
...
fn bench_apply(c: &mut Criterion) {
    ...
    for corpus in Corpus::all() {
        for &scale in &OP_SCALES {
            ...
            g.bench_with_input(
                ...
                |b, _| {
                    b.iter(|| {
                        ...
                        run(apply(&store10, &root10, &cs)).unwrap()
                    })
                },
            );
            ...
            let path = probe.path.clone();
            let upd = tagged_addr(b"upd", probe.path.as_bytes());
            g.bench_with_input(
                BenchmarkId::from_parameter(bench_id(F02, corpus.name(), scale)),
                &scale,
                |b, _| {
                    b.iter(|| {
                        let mut m: PlainManifest<&OldCountingStore> =
                            Manifest::open(root02, &store02);
                        run(m.add(&path, upd)).unwrap();
                        run(m.save()).unwrap()
                    })
                },
            );
