...
#![allow(dead_code, clippy::missing_const_for_fn, clippy::unwrap_used)]
...
pub(crate) type Raw = Arc<MemoryStore<StandardChunkSet>>;
pub(crate) type Store = ContentGet<Raw>;
pub(crate) type Nodes = NodeLoadSaver<Raw>;
pub(crate) type Trie = MantarayManifest<Nodes, Store, DEFAULT_BODY_SIZE>;
pub(crate) type Kv = Database<Store>;

/// A fresh raw store and its content-addressed read handle.
pub(crate) fn stores() -> (Raw, Store) {
...
pub(crate) async fn both_formats(raw: &Raw, store: &Store) -> ((Trie, ChunkRef), (Kv, ChunkRef)) {
...
pub(crate) async fn save_file(raw: &Raw, data: &[u8]) -> ChunkRef {
...
pub(crate) async fn doc_delta<M: Manifest<ChunkRef>>(
    manifest: &M,
    base: &ChunkRef,
...
pub(crate) async fn applied<M: Manifest<ChunkRef>>(
    manifest: &M,
    base: &ChunkRef,
...
pub(crate) async fn write_keys<M: Manifest<ChunkRef>>(
    manifest: &M,
    empty: &ChunkRef,
    keys: &[&str],
) -> ChunkRef {
...
pub(crate) async fn build<M: Manifest<ChunkRef>>(
    manifest: &M,
    empty: &ChunkRef,
...
pub(crate) fn refused<T, F>(result: &Result<T, ManifestError<F>>) -> Option<ManifestPath> {
    result
        .as_ref()
        .err()?
        .as_reserved()
...
pub(crate) async fn key_set<M: Manifest<ChunkRef>>(manifest: &M, root: &ChunkRef) -> Vec<String> {
    drain(&manifest.at(*root), (Bound::Unbounded, Bound::Unbounded)).await
}

/// Every path a bounded walk yields, in order.
pub(crate) async fn drain<V: ManifestView<ChunkRef>>(
    view: &V,
...
pub(crate) async fn refs<M: Manifest<ChunkRef>>(
    manifest: &M,
    root: &ChunkRef,
    cases: &[(&str, Option<u8>)],
    hint: &str,
...
/// and one absent path.
pub(crate) fn probed() -> Vec<ManifestPath> {
    let a = ["", "/", "!", "404.html", "a", "ab"];
...
pub(crate) fn removal_keys() -> Vec<(ManifestPath, u8)> {
...
pub(crate) fn survivors(keys: &[(ManifestPath, u8)], index: usize) -> Vec<(ManifestPath, u8)> {
    keys.iter()
        .enumerate()
...
pub(crate) struct Observed {
    ...
    pub(crate) ranged: Vec<String>,
    pub(crate) listed: Vec<String>,
    pub(crate) prefixed: Vec<String>,
...
pub(crate) struct Probe {
    ...
    pub(crate) entry: Option<MapEntry<ChunkRef>>,
...
/// shape.
pub(crate) async fn observe<M>(manifest: &M, root: &ChunkRef, paths: &[ManifestPath]) -> Observed
where
    M: Manifest<ChunkRef>,
...
pub(crate) async fn observed<M>(manifest: &M, roots: &[&ChunkRef]) -> Vec<Observed>
where
    M: Manifest<ChunkRef>,
    M::Metadata: Clone + PartialEq + std::fmt::Debug,
{
