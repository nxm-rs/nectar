//! Parallel stamping and verification utilities.
//!
//! This module provides high-throughput parallel implementations for:
//! - Stamp signing with sharded bucket state to minimize contention
//! - Stamp verification using rayon parallel iterators
//!
//! # Architecture
//!
//! ## Sharded Stamping
...
impl BucketShard {
    ...
    #[inline]
    fn local_index(&self, bucket: u32) -> usize {
...
/// // Now safe to use from multiple threads via ParallelStamper
/// ```
#[derive(Debug)]
...
impl ShardedIssuer {
    ...
    pub fn with_shard_count(
        ...
        assert!(shard_count.is_power_of_two(), "shard_count must be a power of 2");
    ...
    #[inline]
    fn shard_index(&self, bucket: u32) -> usize {
...
pub struct StampResult {
    ...
    pub result: Result<Stamp, StampError>,
}
...
pub struct VerifyResult {
    ...
    pub result: Result<Address, StampVerifyError>,
}

/// Error during stamp verification.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum StampVerifyError {
    /// The signature could not be recovered.
    RecoveryFailed,
    /// The recovered address does not match the expected owner.
    WrongSigner {
        /// The expected signer address.
        expected: Address,
        /// The actual recovered address.
        recovered: Address,
    },
}

impl std::fmt::Display for StampVerifyError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::RecoveryFailed => write!(f, "signature recovery failed"),
            Self::WrongSigner { expected, recovered } => {
                write!(f, "wrong signer: expected {}, got {}", expected, recovered)
            }
        }
    }
}
...
fn recover_stamp_signer(stamp: &Stamp, address: &SwarmAddress) -> Result<Address, StampVerifyError> {
    ...
    let sig = Signature::from_raw(stamp.signature())
        .map_err(|_| StampVerifyError::RecoveryFailed)?;
...
mod tests {
    ...
    fn test_verify_wrong_signer() {
        ...
        assert!(matches!(
            ...
            Err(StampVerifyError::WrongSigner { .. })
        ));
    }
}
