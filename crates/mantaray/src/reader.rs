...
impl<S: TrustedGet<AnyChunkSet<BS>>, const BS: usize> Reader<S, BS> {
    ...
    /// the path is absent or names an edge rather than a value.
    ///
    /// Fetches the root, one node per matched edge, and the terminal value
    /// node; a non-value terminal is decided from its parent's fork record
        ...
        loop {
            ...
            let (child, terminal) = {
                ...
                let terminal = if next.is_empty() {
                    if !fork.node_type().contains(NodeType::VALUE) {
                        return Ok(None);
                    }
