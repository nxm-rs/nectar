...
use nectar_postage::{Stamp, StampedChunk, Unvalidated};
...
pub(super) fn window(slots: u16) -> Window {
...
pub(super) fn addresses(n: usize) -> Vec<ChunkAddress> {
...
pub(super) struct PanickingSigner;
...
pub(super) struct Gauge {
    pub(super) current: Arc<AtomicUsize>,
...
pub(super) struct SignalWaker(pub(super) mpsc::Sender<()>);
...
/// Drops each job without polling it: the sign task and its sender die unrun,
/// cancelling the completion receiver.
pub(super) struct DroppingSpawner;

impl Spawn for DroppingSpawner {
    fn spawn(&self, _task: BoxFuture<'static, ()>) -> TaskHandle {
        TaskHandle::new(|| {})
    }
}
...
impl<const B: usize> ChunkPut<StampedChunk<Verified, Unvalidated, B>> for CountingSink {
    type Error = core::convert::Infallible;

    async fn put(
        &self,
        stamped: StampedChunk<Verified, Unvalidated, B>,
    ) -> Result<(), Self::Error> {
        self.seen
            .lock()
            .unwrap()
...
impl<const B: usize> ChunkPut<StampedChunk<Verified, Unvalidated, B>> for RefusingSink {
    type Error = SinkRefused;

    async fn put(
        &self,
        _stamped: StampedChunk<Verified, Unvalidated, B>,
    ) -> Result<(), Self::Error> {
        Err(SinkRefused)
    }
}
