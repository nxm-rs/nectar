...
use super::signer::allocates_from;
use super::{BoundSigner, SignPrehash, StampPipeline, StampResult, StampSink};
...
pub struct SealResult<const BODY_SIZE: usize = DEFAULT_BODY_SIZE> {
    ...
    pub result: Result<StampedChunk<Verified, Validated, BODY_SIZE>, SigningError>,
}
    ...
    C: Clock,
{
    ...
    pub fn sign_stage<'p, I, S, const BODY_SIZE: usize>(
        ...
        issuer: &'p I,
        spawner: S,
    ) -> Result<SignStage<'p, Sg, C, I, S, BODY_SIZE>, IssuerError>
    where
        Sg: BoundSigner<Spec = I::Spec>,
        I: StampIssuer + ?Sized,
        S: Spawn,
    {
        allocates_from(issuer, self.signer.as_ref())?;
        Ok(SignStage {
    ...
    S: Spawn,
{
    ...
    fn pair(&mut self, result: StampResult) -> SealResult<BODY_SIZE> {
        ...
        let result = match (result.result, chunk) {
            (Ok(stamp), Some(chunk)) => StampedChunk::new(chunk, stamp)
                .issued_by(self.signer.batch(), self.signer.address())
                .map_err(SigningError::Stamp),
...
mod tests {
    ...
    fn fail_fast_yields_a_not_admitted_tail() {
        let issuer = issuer24();
        let pipeline = StampPipeline::new(bound(FailingSigner)).with_window(window(4));
