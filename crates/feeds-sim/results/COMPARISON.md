...
The harness measures nectar's `latest()` and a faithful port of the reference client's concurrent finder over the same counting presence store, so every figure below is a deterministic work count regenerated by running the harness (`cargo run -p nectar-feeds-sim --bin feeds-perf`), not wall time and not a committed snapshot. The nectar column uses window width 8 to match the reference finder's fixed eight-way lookahead; the reference series has no width sweep because its concurrency is not tunable. The lengths cover every prefill the reference client's own lookup benchmark exercises (1, 100, 1000, 5000) plus longer feeds its benchmark does not reach.
