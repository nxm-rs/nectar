...
use nectar_mantaray::{DefaultMemoryStore, EditorError, ManifestEditor, MantarayError};
...
fuzz_target!(|ops: Vec<FuzzOp>| {
    ...
    // The editor rejects the all-zero reference that the legacy builder
    // silently drops, so a zero-fill put fails the commit only here.
    if let Err(EditorError::Apply {
        index,
    ...
    }) = &first
    {
        assert!(
            matches!(
                ops.get(*index),
                Some(FuzzOp::Put { fill: 0, .. } | FuzzOp::PutMeta { fill: 0, .. })
            ),
            "zero-reference reject at a non-zero put"
        );
        return;
    }
