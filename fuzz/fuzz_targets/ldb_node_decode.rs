//! Fuzz the mantaray 1.0 key-value node decoder with raw attacker-controlled
//! bytes.
//!
//! `Node::decode` is the entry point through which untrusted node chunks
//! from the network are parsed. The shared
//! `nectar_ldb::oracles::node_decode_canonical` oracle holds the
//! reject-or-accept contract on every input: no panic, no OOM, no hang, and
//! an accepted image re-encodes byte-exactly and decodes back to the same
//! node. The decode -> encode bijection proven here on arbitrary input
//! complements the structured round-trip target.
//!
//! Seeds live in `fuzz/seeds/ldb_node_decode/` and are replayed on
//! stable by `seed_replay_ldb_node_decode` in
//! `crates/ldb/src/codec.rs`, through the same oracle.

#![no_main]

use libfuzzer_sys::fuzz_target;
use nectar_ldb::oracles;

fuzz_target!(|data: &[u8]| {
    let _ = oracles::node_decode_canonical(data).expect("an accepted image must be canonical");
});
